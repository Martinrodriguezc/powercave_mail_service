...
function renderPlanRows(
  items: { userName: string; planName: string; expiryDate: string }[],
  dateColor: string,
): string {
  return items
    .map((item, idx) => {
      ...
      return `<tr>
                ...
                <td style="padding:11px 0; color:${dateColor}; font-size:14px; font-weight:600; border-bottom:${border};">${item.expiryDate}</td>
            </tr>`;
    })
    .join("");
}

export function renderDailyAdminReportHTML(
  opts: AdminRenewalReportMail,
): string {
  let html = dailyAdminReportTemplate;

  const hasExpiring =
    Array.isArray(opts.expiringSoon) && opts.expiringSoon.length > 0;
  const hasRecently =
    Array.isArray(opts.recentlyExpired) && opts.recentlyExpired.length > 0;
  ...
  const expiringSection = hasExpiring
    ? `<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
              ...
              <tbody>
                ${renderPlanRows(opts.expiringSoon, "#D4A853")}
              </tbody>
            </table>`
  ...
  const recentlySection = hasRecently
    ? `<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
              ...
              <tbody>
                ${renderPlanRows(opts.recentlyExpired, "#fca5a5")}
              </tbody>
            </table>`
