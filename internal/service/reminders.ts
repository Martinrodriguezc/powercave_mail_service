...
async function hasRecentReminderSent(
  publicId: string,
): Promise<{ hasRecent: boolean; lastSentAt: Date | null }> {
  if (!publicId) {
...
function generateReminderReportHTML(
  ...
  const tableRows = reporte_final
    .map((result) => {
      ...
      if (result.reason) {
        ...
        reasonCell =
          '<td style="padding:10px 8px; border-bottom:1px solid #1e1e1e; color:#6b7280; background-color:#0f0f0f;">-</td>';
      }
...
export async function sendReminderReportEmail(
  ...
  const html = generateReminderReportHTML(
    reporte_final,
    fecha,
    gymName,
    logoUrl,
