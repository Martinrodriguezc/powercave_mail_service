...
router.post("/send_daily_admin_report", requireApiKey, async (req, res) => {
    ...
    const ctx = resolveMailContext(mailData, 'daily_admin_report_backend');

    try {
        await sendDailyAdminReportMail(mailData, ctx);
...
router.post("/send_daily_sales_report", requireApiKey, async (req, res) => {
    const mailData = req.body;
    const ctx = resolveMailContext(mailData, 'sales_registry_backend');

    try {
        await sendDailySalesReportMail(mailData, ctx);
